...
const isAlphabet = value => {
  ...
  const alphabetLookup = base32[alphabetLookupSymbol]
...
const toAlphabet = value => {
  ...
  for (let i = 0; i < 32; i++) {
    ...
    if (alphabetLookup[char] === undefined) {
      throw new SyntaxError(`Invalid character "${char}" at index ${i} for the Base32 alphabet`)
    }
...
class Base32 {
  ...
  decodeInt(string) {
    ...
    for (let i = isNegative && length > 1 ? 1 : 0; i < length; i++) {
      ...
      if (index === undefined) {
        throw new SyntaxError(`Invalid character "${char}" at index ${i} for Base32 encoding`)
      }
  ...
  [decodeFromStringSymbol](string, start, end) {
    ...
    const result = new Uint8Array(validLength * 5 / 8)
    let shift = 8
    let carry = 0
    let index = 0
  ...
  decode(input, start, end) {
    ...
    for (let i = startIndex; i < endIndex; i++) {
      ...
      if (charIndex === undefined) {
        throw new SyntaxError(`Invalid byte "${NumberPrototypeToString(charCode, 16)}" at index ${i} for Base32 encoding`)
      }
      const number = charIndex & 0xff
      shift -= 5
      if (shift > 0) {
...
const isBase32 = FunctionPrototypeBind(FunctionPrototypeSymbolHasInstance, null, Base32)

const base32 = new Base32()
const encodeInt = FunctionPrototypeBind(Base32.prototype.encodeInt, base32)
const decodeInt = FunctionPrototypeBind(Base32.prototype.decodeInt, base32)
const encodeBigInt = FunctionPrototypeBind(Base32.prototype.encodeBigInt, base32)
const decodeBigInt = FunctionPrototypeBind(Base32.prototype.decodeBigInt, base32)
const encodeText = FunctionPrototypeBind(Base32.prototype.encodeText, base32)
const decodeText = FunctionPrototypeBind(Base32.prototype.decodeText, base32)
const encode = FunctionPrototypeBind(Base32.prototype.encode, base32)
const decode = FunctionPrototypeBind(Base32.prototype.decode, base32)
const encodeToString = FunctionPrototypeBind(Base32.prototype.encodeToString, base32)
const decodeFromString = FunctionPrototypeBind(Base32.prototype.decodeFromString, base32)
...
ObjectDefineProperties(Base32.prototype, {
  [SymbolToStringTag]: {
    value: 'Base32'
  }
})
